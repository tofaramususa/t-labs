 ...
 export const Footer = () => (
	<footer className="border-t border-white-a08 mt-12 py-[5.6rem] text-sm">
		<Container className="flex flex-col md:flex-row justify-between">
			...
			<div className="flex flex-wrap">
				{footerLinks.map(column =>
					(
						<div className="min-w-[18rem] min-w-[50%] mt-10 lg:mt-0">
							<h3 className="font-medium mb-3 ">{column.title}</h3>
							<ul>
								{column.links.map( (link) => 
								(
									<li className="[&_a]:last:mb-0">
										<Link  className="mb-3 last:mb-0 block text-grey" href={link.href}>{link.title}</Link>
									</li>
								)
